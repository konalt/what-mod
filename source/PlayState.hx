...
class PlayState extends MusicBeatState
{
	...
	override public function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.SEVEN)
		{
			...
			#if windows
			DiscordClient.changePresence("MAKIN DA SONGS", null, null, true);
			#end
		...
		else
		{
			if (isStoryMode)
			{
				...
				else
				{
					...
					if (StringTools.replace(PlayState.storyPlaylist[0], " ", "-").toLowerCase() == 'eggman')
					{
